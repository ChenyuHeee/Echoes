...
export class RaceScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.distText = this.add.text(14, 8, '距离   0 m', {
      fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#c060ff',
    }).setDepth(21)
    this.sandText = this.add.text(260, 8, '时砂 +0', {
      fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#c8e060',
    }).setDepth(21)
    this.add.text(width / 2, 8, '时隙穿越', {
      fontFamily: '"Silkscreen", monospace', fontSize: '14px', color: '#c060ff',
    }).setOrigin(0.5, 0).setDepth(21)
    this.skill1Txt = this.add.text(width - 190, 8, '[1] 瞬移  CD:就绪', {
      fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#507060',
    }).setDepth(21)
  ...
  private showCountdown() {
    ...
    const next = () => {
      const t = this.add.text(width / 2, height / 2, labels[idx], {
        fontFamily: '"Silkscreen", monospace', fontSize: '80px', color: '#ffffff',
  ...
  private showResult(success: boolean) {
    ...
    this.add.text(width / 2, height / 2 - 84, '时隙崩解', {
      fontFamily: '"Silkscreen", monospace', fontSize: '28px', color: '#ff5030',
    }).setOrigin(0.5).setDepth(71)
    this.add.text(width / 2, height / 2 - 44, `穿越距离　${Math.floor(this.distance)} m`, {
      fontFamily: '"Silkscreen", monospace', fontSize: '18px', color: '#c060ff',
    }).setOrigin(0.5).setDepth(71)
    this.add.text(width / 2, height / 2 - 14, `最高速度　${Math.floor(this.speed)} px/s`, {
      fontFamily: '"Silkscreen", monospace', fontSize: '14px', color: '#506070',
    }).setOrigin(0.5).setDepth(71)
    this.add.text(width / 2, height / 2 + 14, `收集时砂　${this.sandCount}`, {
      fontFamily: '"Silkscreen", monospace', fontSize: '16px', color: '#c8e060',
    }).setOrigin(0.5).setDepth(71)

    const retry = this.add.text(width / 2 - 92, height / 2 + 82, '[ 再次挑战 ]', {
      fontFamily: '"Silkscreen", monospace', fontSize: '15px', color: '#c060ff',
    }).setOrigin(0.5).setDepth(71)
    ...
    const back = this.add.text(width / 2 + 92, height / 2 + 82, '[ 返回大厅 ]', {
      fontFamily: '"Silkscreen", monospace', fontSize: '15px', color: '#607080',
    }).setOrigin(0.5).setDepth(71)
