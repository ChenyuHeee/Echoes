...
export class LoginScene extends Phaser.Scene {
  ...
  create() {
    ...
    const checkingText = this.add.text(width / 2, height / 2, '正在检查登录状态...', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '16px', color: '#4a6a8a',
    }).setOrigin(0.5)
  ...
  private buildLoginUI() {
    ...
    this.add.text(width / 2, height * 0.14, '认证终端', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '36px', color: '#c8a96e',
    }).setOrigin(0.5)

    this.add.text(width / 2, height * 0.26, '使用 Supabase 账号进入在线模式', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '15px', color: '#6080a0',
    }).setOrigin(0.5)

    this.statusText = this.add.text(width / 2, height * 0.35, '状态：未登录', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '13px', color: '#dbe8ff', align: 'center',
    }).setOrigin(0.5)
  ...
  private showInputPanel(
    ...
    for (const f of fields) {
      ...
      inp.style.cssText = [
        ...
        'padding:8px 12px', 'font-family:"Noto Sans SC",monospace', 'font-size:14px',
    ...
    submitBtn.style.cssText = [
      ...
      'color:#90c8e8', 'font-family:"Noto Sans SC",monospace', 'font-size:14px',
    ...
    cancelBtn.style.cssText = [
      ...
      'color:#506070', 'font-family:"Noto Sans SC",monospace', 'font-size:14px',
      'padding:10px', 'cursor:pointer',
    ].join(';')
  ...
  private makeBtn(x: number, y: number, label: string, w: number, onClick: () => void) {
    ...
    this.add.text(x, y, label, {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '14px', color: '#8ab0d0',
    }).setOrigin(0.5)
