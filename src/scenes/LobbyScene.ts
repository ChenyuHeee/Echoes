...
export class LobbyScene extends Phaser.Scene {
  ...
  private createSelectUI() {
    ...
    this.add.text(width / 2, 78, '选择深潜碎片', {
      fontFamily: '"DotGothic16", monospace', fontSize: '13px', color: '#7090b0',
    }).setOrigin(0.5)

    this.buildFragmentCards(width, height)

    this.statusText = this.add.text(width / 2, height - 66, '状态：等待组队', {
      fontFamily: '"DotGothic16", monospace', fontSize: '12px', color: '#c8d8f0',
    }).setOrigin(0.5)
  ...
  private createWaitingRoomUI() {
    ...
    this.add.text(width / 2, 18, '时间深潜 · 等待室', {
      fontFamily: '"DotGothic16", monospace', fontSize: '22px', color: '#c8a96e',
    }).setOrigin(0.5)

    // 房间码（大字）
    this.add.text(width / 2, 56, this.roomCode, {
      fontFamily: '"DotGothic16", monospace', fontSize: '40px', color: '#7ce0bc', letterSpacing: 14,
    }).setOrigin(0.5)

    const theme = FRAGMENT_THEMES[this.mapFragment]
    this.add.text(width / 2, 90, `地图：${theme.name}  ·  将房间码分享给队友`, {
      fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: '#405060',
    }).setOrigin(0.5)
    ...
    this.add.text(width / 2 - 220, panelCY - 98, '▸ 队伍成员', {
      fontFamily: '"DotGothic16", monospace', fontSize: '12px', color: '#4a6a8a',
    }).setOrigin(0, 0.5)
    ...
    this.readyBtnText = this.add.text(readyX, height - 76, '准备', {
      fontFamily: '"DotGothic16", monospace', fontSize: '14px', color: '#60a0d0',
    }).setOrigin(0.5)
    ...
    if (this.isHost) {
      ...
      this.broadcastBtnText = this.add.text(width / 2 - 160, height - 44, '广播到大厅', {
        fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: '#4a7a9a',
      }).setOrigin(0.5)
      ...
      this.add.text(width / 2, height - 44, '催促准备', {
        fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: '#4a7a5a',
      }).setOrigin(0.5)
      ...
      this.add.text(width / 2 + 148, height - 44, '解散房间', {
        fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: '#904040',
      }).setOrigin(0.5)
    ...
    if (!this.isHost) {
      ...
      this.add.text(72, height - 44, '← 离开房间', {
        fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: '#704040',
  ...
  private renderPlayerList() {
    ...
    for (const [id, player] of this.players) {
      ...
      const nameText = this.make.text({
        ...
        style: { fontFamily: '"DotGothic16", monospace', fontSize: '15px', color: readyColor },
        add: false,
      }).setOrigin(0, 0.5)
      ...
      const badge = this.make.text({
        ...
        style: { fontFamily: '"DotGothic16", monospace', fontSize: '12px', color: player.ready ? '#5adc9a' : '#404c5a' },
        add: false,
      }).setOrigin(1, 0.5)
      ...
      if (this.isHost && !isMe) {
        const kickBtn = this.make.text({
          x: width / 2 + 163, y,
          text: '[踢]',
          style: { fontFamily: '"DotGothic16", monospace', fontSize: '10px', color: '#703030' },
          add: false,
  ...
  private buildFragmentCards(width: number, height: number) {
    ...
    FRAGMENT_OPTIONS.forEach((opt, i) => {
      ...
      container.add(this.make.text({
        ...
        style: { fontFamily: '"DotGothic16", monospace', fontSize: '13px', color: theme.ambientColor },
        add: false,
      }).setOrigin(0.5).setAlpha(isSelected ? 1 : 0.5))

      container.add(this.make.text({
        x: 0, y: -CARD_H / 2 + 42, text: theme.name,
        style: { fontFamily: '"DotGothic16", monospace', fontSize: '18px', color: isSelected ? '#e8f0ff' : '#506070' },
        add: false,
      }).setOrigin(0.5))

      container.add(this.make.text({
        x: 0, y: -CARD_H / 2 + 62, text: theme.subtitle,
        style: { fontFamily: '"DotGothic16", monospace', fontSize: '11px', color: theme.ambientColor },
        add: false,
  ...
  private showJoinPanel() {
    ...
    inp.style.cssText = [
      ...
      'padding:10px 14px', 'font-family:"DotGothic16",monospace', 'font-size:20px',
    ...
    joinBtn.style.cssText = [
      ...
      'color:#90c8e8', 'font-family:"DotGothic16",monospace', 'font-size:14px', 'padding:10px', 'cursor:pointer',
    ].join(';')
    ...
    cancelBtn.style.cssText = [
      ...
      'color:#405060', 'font-family:"DotGothic16",monospace', 'font-size:14px', 'padding:10px', 'cursor:pointer',
    ].join(';')
