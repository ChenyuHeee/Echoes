...
export class LoadoutScene extends Phaser.Scene {
  ...
  init(data: LoadoutInit) {
    ...
    // 默认全选仓库装备
    const stash = getRuntimeState().player.stash ?? { weaponId: null, attachmentIds: [], itemIds: [] }

    this.selectedWeapon = stash.weaponId
      ? ((WEAPON_DEFINITIONS as Record<string, WeaponDef | undefined>)[stash.weaponId] ?? null)
      : null

    this.selectedAttachments = new Set(stash.attachmentIds)
  ...
  private buildUI() {
    ...
    if (weapDef) {
      this.addToggleRow(
        ...
        () => {
          this.selectedWeapon = this.selectedWeapon ? null : weapDef
          this.buildUI()
        },
    ...
    if (stashAttDefs.length === 0) {
      ...
      ATTACHMENT_SLOTS.forEach(slot => {
        const att = stashAttDefs.find(a => a.slotType === slot)
        if (!att) return
        this.addToggleRow(
          lx, ly, LEFT_W, att,
          () => this.selectedAttachments.has(att.id),
          () => {
            if (this.selectedAttachments.has(att.id)) {
              this.selectedAttachments.delete(att.id)
            } else {
              this.selectedAttachments.add(att.id)
            }
            this.buildUI()
          },
          `[${slotNames[slot]}] ${att.name}  [${RARITY_NAMES[att.rarity]}]`,
    ...
    ATTACHMENT_SLOTS.forEach(slot => {
      ...
      this.container.add(this.make.text({
        x: rx + 6, y: ry + 5,
        text: att ? `[${slotLabel}] ${att.name}` : `[${slotLabel}] 空`,
