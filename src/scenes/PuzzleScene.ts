...
export class PuzzleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.roomTitle    = this.add.text(480, 8,  '', { fontFamily: '"Silkscreen", monospace', fontSize: '15px', color: '#50e8a0' }).setOrigin(0.5, 0).setScrollFactor(0).setDepth(31)
    this.levelCounter = this.add.text(480, 27, '', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#304860' }).setOrigin(0.5, 0).setScrollFactor(0).setDepth(31)

    const back = this.add.text(14, 8, '<- 返回', { fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#304050' }).setOrigin(0, 0).setScrollFactor(0).setDepth(31)
    ...
    this.echoText = this.add.text(700, 8, 'P1回响：空', { fontFamily: '"Silkscreen", monospace', fontSize: '11px', color: '#384850' }).setOrigin(0, 0).setScrollFactor(0).setDepth(31)
    this.hintText = this.add.text(480, 510, '', { fontFamily: '"Silkscreen", monospace', fontSize: '11px', color: '#3a5868', wordWrap: { width: 920 }, align: 'center' }).setOrigin(0.5, 1).setScrollFactor(0).setDepth(31)
    this.statusText = this.add.text(480, 290, '', { fontFamily: '"Silkscreen", monospace', fontSize: '17px', color: '#ffffff', stroke: '#000000', strokeThickness: 4 }).setOrigin(0.5).setScrollFactor(0).setDepth(40).setAlpha(0)
    this.keyDisplay  = this.add.text(860, 8, '', { fontFamily: '"Silkscreen", monospace', fontSize: '11px', color: '#f0c060' }).setOrigin(1, 0).setScrollFactor(0).setDepth(31)
    this.portalHint  = this.add.text(480, 527, this.isCommunityMode ? 'Q键: 发射传送门  ·  WASD: 移动' : '', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#304050' }).setOrigin(0.5, 1).setScrollFactor(0).setDepth(31)
  ...
  private buildLevel() {
    ...
    level.pads.forEach(p => {
      ...
      const label = this.add.text(p.x, p.y, p.id, { fontFamily: '"Silkscreen", monospace', fontSize: '22px', color: p.color }).setOrigin(0.5).setDepth(6)
    ...
    level.doors.forEach(d => {
      ...
      const label   = this.add.text(d.x, d.y + d.h / 2 + 10, `${reqStr}\n${d.windowMs}ms`, { fontFamily: '"Silkscreen", monospace', fontSize: '9px', color: '#8850d0', align: 'center' }).setOrigin(0.5, 0).setDepth(9)
  ...
  private setupPlayer2() {
    ...
    this.echo2Text = this.add.text(700, 25, 'P2回响：空', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#7a6020' }).setOrigin(0, 0).setScrollFactor(0).setDepth(31)
  }
  ...
  private buildCommunityLevel() {
    ...
    for (const el of map.elements) {
      switch (el.type) {
        ...
        case 'label':          this.add.text(el.x, el.y, el.text, { fontFamily: '"Silkscreen", monospace', fontSize: `${el.fontSize ?? 11}px`, color: el.color ?? '#607080', wordWrap: { width: 280 } }).setOrigin(0.5).setDepth(7); break
      }
    }
  ...
  private addCommunityPad(id: string, x: number, y: number, color: string, isTrap: boolean) {
    ...
    const label = this.add.text(x, y, id, { fontFamily: '"Silkscreen", monospace', fontSize: '20px', color }).setOrigin(0.5).setDepth(6)
    if (isTrap) this.add.text(x, y + 28, '⚠', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#c05050' }).setOrigin(0.5).setDepth(6)
  ...
  private addCommunityDoor(el: { id?: string; x: number; y: number; w: number; h: number; requires: string[]; windowMs: number }) {
    ...
    const label   = this.add.text(el.x, el.y, reqStr, { fontFamily: '"Silkscreen", monospace', fontSize: '9px', color: '#8850d0', align: 'center' }).setOrigin(0.5).setDepth(9)
  ...
  private addPressurePlate(el: { id: string; x: number; y: number; linksTo: string[]; requireAll?: boolean; color?: string }) {
    ...
    const label  = this.add.text(el.x, el.y - 14, '▬', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: hexCol }).setOrigin(0.5).setDepth(6)
  ...
  private addSwitch(x: number, y: number, linksTo: string[], mode: string, color: string) {
    ...
    const label  = this.add.text(x, y - 18, '⚡', { fontFamily: '"Silkscreen", monospace', fontSize: '11px', color: hexCol }).setOrigin(0.5).setDepth(6)
  ...
  private addTimedDoor(el: { id?: string; x: number; y: number; w: number; h: number; openMs: number; closeMs: number; initialState?: string; phaseOffset?: number }) {
    ...
    const label     = this.add.text(el.x, el.y, startOpen ? '○' : '●', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#f08040' }).setOrigin(0.5).setDepth(9)
  ...
  private addConveyor(x: number, y: number, w: number, h: number, vx: number, vy: number, color: string) {
    ...
    this.add.text(x, y, arrow, { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#506070' }).setOrigin(0.5).setDepth(6)
    const zone = this.add.zone(x, y, w, h)
    this.physics.add.existing(zone, true)
  ...
  private addKeyDoor(el: { id?: string; x: number; y: number; w: number; h: number; keyId: string; color?: string }) {
    ...
    const label  = this.add.text(el.x, el.y, '🔒', { fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: hexCol }).setOrigin(0.5).setDepth(9)
  ...
  private addPortalSurface(id: string, x: number, y: number, w: number, h: number) {
    ...
    this.add.text(x, y, '▨', { fontFamily: '"Silkscreen", monospace', fontSize: '10px', color: '#2050a0' }).setOrigin(0.5).setDepth(6)
  ...
  private showCommunityExit() {
    ...
    this.exitTxt = this.add.text(x, y - 38, '出口', { fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#ffd060' }).setOrigin(0.5, 1).setDepth(13)
  ...
  private showExitPortal() {
    ...
    this.exitTxt = this.add.text(exitX, exitY - 36, '出口', { fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#ffd060' }).setOrigin(0.5, 1).setDepth(13)
    this.tweens.add({ targets: this.exitTxt, y: this.exitTxt.y - 6, duration: 800, yoyo: true, repeat: -1 })
  ...
  private showVictory() {
    ...
    this.add.text(W / 2, H / 2 - 90, isCom ? '✦ 社区地图全部通关 ✦' : '✦ 时序密室全部破解 ✦', { fontFamily: '"Silkscreen", monospace', fontSize: '22px', color: '#50e8a0' }).setOrigin(0.5).setScrollFactor(0).setDepth(121)
    const total = isCom ? COMMUNITY_MAPS.length : (this.isCoop ? 5 : 30)
    this.add.text(W / 2, H / 2 - 52, `通关 ${total} 个关卡`, { fontFamily: '"Silkscreen", monospace', fontSize: '14px', color: '#507090' }).setOrigin(0.5).setScrollFactor(0).setDepth(121)
    this.add.text(W / 2, H / 2 - 20, `获得时砂  ${this.totalSand}`, { fontFamily: '"Silkscreen", monospace', fontSize: '18px', color: '#c8e060' }).setOrigin(0.5).setScrollFactor(0).setDepth(121)
    this.add.text(W / 2, H / 2 + 14, '回响不止于战斗——它是时间本身的语言', { fontFamily: '"Silkscreen", monospace', fontSize: '12px', color: '#384850' }).setOrigin(0.5).setScrollFactor(0).setDepth(121)
    const mkBtn = (cx: number, label: string, color: number, fn: () => void) => {
      ...
      this.add.text(cx, H / 2 + 76, label, { fontFamily: '"Silkscreen", monospace', fontSize: '14px', color: '#' + color.toString(16).padStart(6, '0') }).setOrigin(0.5).setScrollFactor(0).setDepth(122)
    }
