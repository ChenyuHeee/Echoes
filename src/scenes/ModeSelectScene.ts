...
export class ModeSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, 10, '选择游戏模式', {
      fontFamily: 'monospace', fontSize: '24px', color: '#c8a96e',
    }).setOrigin(0.5, 0)
    this.add.text(width / 2, 36, 'ECHOES: FRACTURED TIME  ·  六种回响，六种宿命', {
      fontFamily: 'monospace', fontSize: '10px', color: '#364858',
    }).setOrigin(0.5, 0)
  ...
  private makeCard(
    ...
    this.add.text(x + w / 2 - 8, y - h / 2 + 8, m.tag, {
      fontFamily: 'monospace', fontSize: '10px', color: tagColor,
    ...
    if (!m.locked) {
      ...
      this.add.text(btnX, btnY, '进入模式', {
        fontFamily: 'monospace', fontSize: '12px', color: m.color,
      ...
      this.add.text(x + w / 2 - 50, y + h / 2 - 16, '联机功能开发中', {
        fontFamily: 'monospace', fontSize: '10px', color: '#2a3840',
