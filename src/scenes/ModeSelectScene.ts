...
export class ModeSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, 10, '选择游戏模式', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '24px', color: '#c8a96e',
    }).setOrigin(0.5, 0)
    this.add.text(width / 2, 36, 'ECHOES: FRACTURED TIME  ·  六种回响，六种宿命', {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '10px', color: '#506878',
    }).setOrigin(0.5, 0)
  ...
  private makeCard(
    ...
    this.add.text(x + w / 2 - 8, y - h / 2 + 8, m.tag, {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '10px', color: tagColor,
    }).setOrigin(1, 0).setDepth(3)

    // 标题
    this.add.text(lx, y - h / 2 + 8, m.title, {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '16px',
      color: m.locked ? '#3a4a58' : m.color,
    }).setOrigin(0, 0).setDepth(3)

    // 副标题
    this.add.text(lx, y - h / 2 + 28, m.sub, {
      fontFamily: '"Noto Sans SC", monospace', fontSize: '10px', color: '#5a7088',
    }).setOrigin(0, 0).setDepth(3)
    ...
    if (!m.locked) {
      ...
      this.add.text(btnX, btnY, '进入模式', {
        fontFamily: '"Noto Sans SC", monospace', fontSize: '12px', color: m.color,
